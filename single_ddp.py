...
def setup(rank, world_size):
    ...
    # os.environ['MASTER_ADDR'] = '172.0.0.1'
    ...
    for image in range(1, len(img_df.index) + 1):
        ...
        for layer in range(len(hidden_layer_list)):
            ...
            for neuron in range(len(neuron_list)):
                ...
                # save image
                img_path = os.path.join(dir_path / img_dir, f'reconstructed_image_{hidden_layer_list[layer]}_{neuron_list[neuron]}_ddp.jpg')
        ...
        heatmap_image_path = os.path.join(dir_path / img_dir, f'psnr_heatmap_image_{image}_ddp.jpg')
        heat_map_psnr.figure.savefig(heatmap_image_path)
        plt.close()

        # generate time heatmap and save
        plt.figure(figsize=(15, 8))
        heat_map_time = sns.heatmap(train_time_df, cmap='GnBu', annot=False, annot_kws={'size':10}, fmt='.3f', xticklabels=True, yticklabels=True)
        plt.xlabel('neurons')
        plt.ylabel('layers')
        plt.title('time_heatmap_img' + str(image))

        heatmap_image_path = os.path.join(dir_path / img_dir, f'time_heatmap_image_{image}_ddp.jpg')
